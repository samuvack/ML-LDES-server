...
def (sensor_name):
    sql = "select * from (select replace(Replace(temperatuur_date, 'T', ' '), 'Z', '')::timestamp as time, temperatuur_value as temperature, conductiviteit_value as conductivity,  ROW_NUMBER() OVER (ORDER BY id) as row from public.water_quality where temperatuur_sensor = " + sensor_name + " order by replace(Replace(temperatuur_date, 'T', ' '), 'Z', '')::timestamp) as q where q.row > 4500"
    dat = sqlio.read_sql_query(sql, conn)
    dat.insert(0, 'id', range(0, 0 + len(dat)))
    return dat

sql = "select * from (select replace(Replace(temperatuur_date, 'T', ' '), 'Z', '')::timestamp as time, temperatuur_value as temperature, conductiviteit_value as conductivity,  ROW_NUMBER() OVER (ORDER BY id) as row from public.water_quality where temperatuur_sensor = 'urn:ngsi-v2:cot-imec-be:Device:aqf-iow-JX3CPbvBck498C3uan9KNg' order by replace(Replace(temperatuur_date, 'T', ' '), 'Z', '')::timestamp) as q where q.row > 4500"
